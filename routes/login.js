...
router.post('/reset-password-link', async (req, res) => {
  ...
  try {
    ...
    const expiresAt = Date.now() + 60000; // 1 min
    ...
    res.send('<script>alert("A password reset link has been sent to your email address. Please check your inbox."); window.location.href="/login";</script>');
  } catch (error) {
    console.error(error.message);
    res.status(500).send('Server error');
  }
...
router.get('/reset-password/:token', async (req, res) => {
  try {
    ...
    if (!passwordResetToken) {
      return res.redirect('/login?error=The password reset link is invalid or has expired. Please request a new one.');
    }
...
router.post('/reset-password/:token', async (req, res) => {
  ...
  if (password !== confirm_password) {
    return res.redirect(`/reset-password/${req.params.token}?error=Password reset failed. The passwords you entered do not match. Please try again.`);
  }
  ...
  try {
    ...
    res.redirect('/login?success=Password updated successfully! You can now proceed to log in with your new credentials.');
  } catch (error) {
    console.error(error.message);
    res.status(500).send('Server error');
  }
